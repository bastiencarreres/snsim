...
def write_sim(wpath, name, formats, header, data):
    ...
    if "parquet" in formats and imp_pyarrow and imp_json:
        ...
        lcmeta = json.dumps(
            {int(data.attrs[k]["ID"]): data.attrs[k] for k in data.attrs}, cls=NpEncoder
        )
...
def read_sim_file(file_path, engine="pyarrow"):
    ...
    if file_ext == ".pkl":
        ...
        if engine == "pyarrow" and imp_pyarrow:
            ...
            warnings.warn(
                "You need pyarrow or fastparquet and json module to read parquet formats",
                UserWarning,
            )
        lcs = table.to_pandas()
        lcs.set_index(["ID", "epochs"], inplace=True)
        lcs.attrs = {int(k): val for k, val in json.loads(hdic[attrs_key]).items()}
